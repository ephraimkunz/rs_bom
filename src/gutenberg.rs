...
impl GutenbergParser {
    pub fn new(path: &path::Path) -> GutenbergParser {
        GutenbergParser { path: path.into() }
    }
}

impl BOMParser for GutenbergParser {
    type Err = GutenbergParseError;
    fn parse(self) -> Result<BOM, Self::Err> {
        ...
        if bom.books.is_empty() {
            return Err(GutenbergParseError::CorpusInvalid(
...
const THREE_WITNESS_TEXT: &str = "Be it known unto all nations, kindreds, tongues, and people, unto
...
const EIGHT_WITNESS_TEXT: &str = "Be it known unto all nations, kindreds, tongues, and people, unto
...
mod tests {
    ...
    fn error_for_empty_document() {
        let parser = GutenbergParser::new(path::Path::new("testdata/empty_file.txt"));
        assert!(parser.parse().is_err())
    }

    #[test]
    fn error_for_invalid_path() {
        let parser = GutenbergParser::new(path::Path::new("testing123"));
        assert!(parser.parse().is_err())
    }

    #[test]
    fn error_for_invalid_document() {
        let parser = GutenbergParser::new(path::Path::new("testdata/bad_data_file.txt"));
        assert!(parser.parse().is_err())
    }
}
